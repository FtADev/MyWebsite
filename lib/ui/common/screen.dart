class Screen {
  final marginTop = 0.0;
  final marginLeft = 0.0;
  final bioFont1 = 0.0;
  final bioFont2 = 0.0;
  final bioFont3 = 0.0;
  final flatBtnFont = 0.0;
  final boxSizeMedium = 0.0;
  final boxSizeLarge = 0.0;
  final iconSize = 0.0;
  final projectWholePadding = 0.0;
  final projectBottomPadding = 0.0;
  final projectCardPadding = 0.0;
  final projectTitleFontSize = 0.0;
  final projectTextFontSize = 0.0;
  final projectSpace = 0.0;
  final projectLangSpace = 0.0;
  final projectTeamWorkSize = 0.0;
  final projectTeamWorkFont = 0.0;
}
